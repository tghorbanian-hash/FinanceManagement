...
const Button = ({ children, variant = 'primary', size = 'md', isLoading = false, disabled = false, icon: Icon, iconPosition = 'right', className = '', onClick, type = 'button', title, ...props }) => {
  ...
  const sizes = { sm: `h-7 ${hasText ? 'px-2.5 gap-1 text-[11px]' : 'w-7'}`, md: `h-8 ${hasText ? 'px-3 gap-1.5 text-[11px]' : 'w-8'}`, lg: `h-10 ${hasText ? 'px-4 gap-2 text-[12px]' : 'w-10'}` };
...
const TextField = ({ label, error, hint, icon: Icon, disabled = false, required = false, className = '', wrapperClassName = '', id, type = 'text', size = 'md', isRtl = true, ...props }) => {
  ...
  const inputHeights = { sm: 'h-7 text-[11px]', md: 'h-8 text-[11px]', lg: 'h-10 text-[12px]' };
  ...
  return (
    <div className={`flex flex-col ${size === 'sm' ? 'gap-1' : 'gap-1.5'} w-full ${wrapperClassName}`}>
      ...
      <div className="relative flex items-center" onClick={() => !disabled && setIsOpen(true)}>
        <div className={`absolute ${isRtl ? 'left-2.5' : 'right-2.5'} text-slate-400 pointer-events-none`}><Search size={14} /></div>
        <div className={`w-full ${size === 'sm' ? 'h-7 text-[11px]' : 'h-8 text-[11px]'} bg-white border rounded-lg text-slate-800 transition-all outline-none flex items-center ${disabled ? 'bg-slate-100/50 text-slate-500 cursor-not-allowed border-slate-200' : 'cursor-pointer hover:border-indigo-400 border-slate-300'} ${isRtl ? 'pr-2 pl-8' : 'pl-2 pr-8'}`}>
          <span className="truncate">{displayValue || placeholder || t('انتخاب...', 'Select...')}</span>
        </div>
      </div>
...
const Card = ({ title, action, children, className = '', noPadding = false, headerClassName = '', isCollapsible = false, defaultCollapsed = false, language = 'fa' }) => {
  ...
  return (
    <div className={`bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex flex-col transition-all ${className}`}>
      {(title || action) && (
        <div 
          className={`h-9 border-b border-slate-100 flex items-center justify-between px-3 bg-slate-50/50 shrink-0 transition-colors ${isCollapsible ? 'cursor-pointer select-none hover:bg-slate-100/50' : ''} ${headerClassName}`}
          onClick={() => isCollapsible && setCollapsed(!collapsed)}
        >
          <div className="flex items-center gap-2 flex-1 min-w-0">
            {isCollapsible && (
              <span className="text-slate-400 shrink-0 transition-transform">
                {collapsed ? (isRtl ? <ChevronLeft size={14}/> : <ChevronRight size={14}/>) : <ChevronDown size={14}/>}
              </span>
            )}
  ...
  return (
    <div className="flex flex-col gap-1.5 mb-3 shrink-0" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className="flex items-center gap-2 text-slate-800">
        {Icon && <div className="p-1.5 bg-white border border-slate-200 shadow-sm text-indigo-600 rounded-lg shrink-0"><Icon size={16} strokeWidth={2.5}/></div>}
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 font-sans" dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      <div className={`bg-white shadow-2xl flex flex-col relative z-10 transition-all duration-300 animate-in zoom-in-95 overflow-hidden ${isMaximized ? 'w-full h-full inset-0 rounded-none' : `${width} w-full max-h-[90vh] rounded-xl`}`}>
        <div className="h-10 px-4 border-b border-slate-200 flex items-center justify-between bg-slate-50 shrink-0">
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg shadow-sm flex flex-col shrink-0 mb-3 font-sans transition-all duration-300" dir={isRtl ? 'rtl' : 'ltr'}>
      <div className="h-9 px-4 flex items-center justify-between cursor-pointer bg-slate-50/50 hover:bg-slate-50 rounded-lg transition-colors select-none" onClick={() => setIsOpen(!isOpen)}>
        ...
        <div className="text-slate-400">{isOpen ? <ChevronUp size={14} /> : <ChevronDown size={14} />}</div>
      </div>
      {isOpen && (
        <div className="p-3 border-t border-slate-100 animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
            {fields.map((f, idx) => {
              ...
              return <TextField key={idx} size="sm" label={f.label} isRtl={isRtl} type={f.type} value={values[f.name] || ''} onChange={(e) => handleChange(f.name, e.target.value)} dir={!isRtl ? 'ltr' : 'rtl'} />;
            })}
          </div>
...
const AttachmentManager = ({ files = [], onUpload, onDelete, onDownload, readOnly = false, language = 'fa' }) => {
  ...
  return (
    <div className="flex flex-col gap-3 font-sans w-full h-full" dir={isRtl ? 'rtl' : 'ltr'}>
      {!readOnly && (
        <div onDragOver={e => {e.preventDefault(); setIsDragging(true);}} onDragLeave={e => {e.preventDefault(); setIsDragging(false);}} onDrop={handleDrop} onClick={() => fileInputRef.current?.click()} className={`flex flex-col items-center justify-center p-3 border-2 border-dashed rounded-xl cursor-pointer transition-all shrink-0 ${isDragging ? 'border-indigo-500 bg-indigo-50' : 'border-slate-300 bg-slate-50 hover:bg-slate-100 hover:border-indigo-400'}`}>
          <UploadCloud size={20} className={isDragging ? 'text-indigo-600' : 'text-slate-400'} />
          <span className="text-[11px] font-bold text-slate-700 mt-2">{t('فایل‌ها را اینجا رها کنید یا کلیک کنید', 'Drop files here or click')}</span>
        ...
        {files.length === 0 ? <div className="text-center p-4 text-[11px] text-slate-400 border border-slate-100 rounded-lg bg-slate-50/50">{t('هیچ فایلی ضمیمه نشده است.', 'No attachments found.')}</div> : files.map((file, idx) => (
          <div key={idx} className="flex items-center justify-between p-2 border border-slate-200 rounded-lg bg-white hover:border-indigo-200 transition-colors group shrink-0">
            ...
            <div className="flex items-center gap-1 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity">
              {onDownload && <button onClick={(e) => { e.stopPropagation(); onDownload(file); }} className="p-1 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-md"><Download size={12} /></button>}
              {!readOnly && onDelete && <button onClick={(e) => { e.stopPropagation(); onDelete(file); }} className="p-1 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-md"><Trash2 size={12} /></button>}
            </div>
          </div>
        ))}
      </div>
...
const Tabs = ({ tabs = [], activeTab, onChange, className = '' }) => {
  return (
    <div className={`flex items-center gap-1 border-b border-slate-200 mb-3 overflow-x-auto custom-scrollbar ${className}`}>
      {tabs.map(tab => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2.5 text-[11px] font-bold transition-all border-b-2 outline-none whitespace-nowrap ${isActive ? 'border-indigo-600 text-indigo-600 bg-indigo-50/50 rounded-t-lg' : 'border-transparent text-slate-500 hover:text-slate-700 hover:bg-slate-50 rounded-t-lg'}`}
          >
            {Icon && <Icon size={14} />}
            {tab.label}
          </button>
...
const DataGrid = ({ data = [], columns = [], actions = [], language = 'fa', onAdd, onRowDoubleClick, selectable = false, bulkActions = [], rowReorderable = false, onRowReorder, onDownloadSample }) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg shadow-sm flex flex-col font-sans h-full overflow-hidden" dir={isRtl ? 'rtl' : 'ltr'}>
      <div className="flex flex-wrap items-stretch p-1 border-b border-slate-200 bg-white gap-2 shrink-0 min-h-[42px]">
        <div className="flex items-center shrink-0">
          {onAdd && (
            <Button size="sm" variant="primary" icon={Plus} onClick={onAdd} className="h-full px-3 shadow-sm">
              {t('جدید', 'New')}
            </Button>
          )}
        </div>
        ...
        {selectedRows.length > 0 && bulkActions.length > 0 ? (
          <div className="flex-1 flex items-center gap-2 px-3 py-1 border border-indigo-200 bg-indigo-50 rounded-md transition-all animate-in fade-in">
            <span className="text-[10px] font-black text-indigo-800">{selectedRows.length} {t('مورد انتخاب شده', 'Items selected')}</span>
            <div className="w-px h-4 bg-indigo-200 mx-1"></div>
            {bulkActions.map((act, i) => (
              <Button key={i} size="sm" variant={act.variant || 'outline'} icon={act.icon} onClick={() => {act.onClick(selectedRows); setSelectedRows([]);}} className="!h-6 text-[10px] px-2">
                {act.label}
              </Button>
            ))}
          </div>
          ...
          <div className={`flex-1 flex items-center gap-2 px-3 py-1 border border-dashed rounded-md transition-colors overflow-x-auto custom-scrollbar ${groupCols.length > 0 ? 'bg-indigo-50/30 border-indigo-200' : 'bg-slate-50 border-slate-200 hover:bg-slate-100'}`} onDragOver={(e) => e.preventDefault()} onDrop={handleGroupDrop}>
            ...
            {groupCols.length === 0 ? (
              <span className="text-[10px] text-slate-400 font-medium whitespace-nowrap">{t('هدر ستون را برای گروه‌بندی اینجا رها کنید', 'Drop column header here to group')}</span>
            ) : (
              <div className="flex items-center gap-2 flex-1 min-w-max">
                {groupCols.map((field, idx) => {
                  const colDef = columns.find(c => c.field === field);
                  return (
                    <div key={field} className="flex items-center gap-1 bg-white border border-indigo-100 text-indigo-700 px-1.5 py-0.5 rounded text-[10px] font-bold shadow-sm">
                      <span>{colDef ? t(colDef.header_fa, colDef.header_en) : field}</span>
                      <button onClick={() => removeGroupCol(field)} className="text-indigo-400 hover:text-red-500 rounded-full p-0.5 ml-1"><X size={10} /></button>
                      {idx < groupCols.length - 1 && <ChevronLeft size={10} className="text-slate-300 ml-1" />}
                    </div>
                  );
                })}
        ...
        <div className="flex items-center gap-1 shrink-0">
          <div className="relative flex items-center h-full" ref={colMenuRef}>
            <button onClick={() => setShowColMenu(!showColMenu)} title={t('نمایش/مخفی‌سازی ستون‌ها', 'Columns')} className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-slate-100 border border-transparent hover:border-slate-200 rounded transition-all h-full flex items-center justify-center"><Settings size={14} /></button>
            {showColMenu && (
              <div className="absolute top-full mt-1 bg-white border border-slate-200 shadow-xl rounded-lg p-2 z-50 min-w-[200px] right-0 animate-in zoom-in-95 duration-100">
                <div className="text-[11px] font-black text-slate-800 mb-2 pb-2 border-b border-slate-100 px-1">{t('نمایش / مخفی‌سازی', 'Show / Hide')}</div>
                <div className="max-h-[250px] overflow-y-auto custom-scrollbar space-y-0.5">
                  {columns.map(c => (
                    <label key={c.field} className="flex items-center gap-2 cursor-pointer p-1 hover:bg-slate-50 rounded text-[10px] font-bold text-slate-600 transition-colors">
                      <input type="checkbox" checked={!hiddenCols.has(c.field)} onChange={() => toggleVisibility(c.field)} className="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 w-3 h-3" />
          ...
          {onDownloadSample && (
            <button onClick={onDownloadSample} title={t('دانلود نمونه فایل', 'Download Sample')} className="p-1 text-slate-500 hover:text-emerald-600 hover:bg-slate-100 border border-transparent hover:border-slate-200 rounded transition-all h-full flex items-center justify-center"><FileDown size={14} /></button>
          )}
          <button onClick={() => document.getElementById('grid-import-input').click()} title={t('ورود اطلاعات', 'Import')} className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-slate-100 border border-transparent hover:border-slate-200 rounded transition-all h-full flex items-center justify-center"><Upload size={14} /></button>
          <input id="grid-import-input" type="file" className="hidden" accept=".csv" />
          <button onClick={exportCSV} title={t('خروجی اکسل', 'Export')} className="p-1 text-slate-500 hover:text-emerald-600 hover:bg-slate-100 border border-transparent hover:border-slate-200 rounded transition-all h-full flex items-center justify-center"><FileSpreadsheet size={14} /></button>
        </div>
      </div>
      ...
      <div className="overflow-auto custom-scrollbar flex-1 relative bg-white">
        <table className="w-full text-start border-separate border-spacing-0 min-w-max" dir={isRtl ? 'rtl' : 'ltr'}>
          <thead className="sticky top-0 z-40 bg-slate-100 shadow-sm">
            <tr>
              {rowReorderable && <th style={{ width: '30px', ...getStickyStyles('ROW_REORDER_COL', false, true) }} className={`p-1 border-b border-slate-200 bg-slate-100 ${isRtl ? 'border-l' : 'border-r'}`}></th>}
              {selectable && (
                <th style={{ width: '40px', ...getStickyStyles('SELECT_COL', false, true) }} className={`p-1 border-b border-slate-200 text-center bg-slate-100 ${isRtl ? 'border-l' : 'border-r'}`}>
              ...
              {visibleColumns.map((col, index) => {
                ...
                return (
                  <th 
                    ...
                    className={`p-1.5 border-b border-slate-200 text-[10px] font-black text-slate-700 select-none bg-slate-100 ${isRtl ? 'border-l' : 'border-r'}`}
                  >
                    <div className="flex items-center justify-between gap-1 group">
                      <div className="flex items-center gap-1 cursor-pointer flex-1 overflow-hidden" onClick={() => handleSort(col.field)}>
                        <GripVertical size={10} className="text-slate-300 opacity-0 group-hover:opacity-100 cursor-grab active:cursor-grabbing shrink-0" />
                        <span className="truncate">{t(col.header_fa, col.header_en)}</span>
                        {sortConfig.field === col.field && <span className="text-indigo-500 shrink-0">{sortConfig.direction === 'asc' ? <ChevronUp size={10}/> : <ChevronDown size={10}/>}</span>}
                      </div>
              ...
              {actions.length > 0 && (
                <th style={{...getStickyStyles('ACTIONS', true, true)}} className="p-1.5 border-b border-slate-200 text-[10px] font-black text-slate-700 w-[100px] bg-slate-100 text-center shadow-[-4px_0_10px_rgba(0,0,0,0.03)]">
                  {t('عملیات', 'Actions')}
                </th>
              )}
            </tr>
              ...
              {visibleColumns.map((col) => {
                return (
                  <td key={`filter-${col.field}`} style={getStickyStyles(col.field, false, true)} className={`p-1 border-b border-slate-200 bg-slate-50 ${isRtl ? 'border-l' : 'border-r'}`}>
                    <div className="relative">
                      {(col.type === 'text' || col.type === 'number') && <Search size={10} className={`absolute top-1/2 -translate-y-1/2 ${isRtl ? 'right-1' : 'left-1'} text-slate-400`} />}
                      {col.type !== 'toggle' && col.type !== 'checkbox' && (
                        <input 
                          ...
                          className={`w-full h-6 text-[10px] font-sans font-bold bg-white border border-slate-200 rounded outline-none focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 transition-all ${col.type === 'date' ? 'px-1' : (isRtl ? 'pr-4 pl-1' : 'pl-4 pr-1')}`}
                        />
                      )}
                    </div>
                  </td>
          ...
          <tbody className="z-10 relative">
            {paginatedData.length > 0 ? paginatedData.map((row, rowIndex) => {
                ...
                return (
                  <tr key={`group-${row.groupKey}`} className="bg-indigo-50/40 border-b border-indigo-100">
                    <td colSpan={visibleColumns.length + (actions.length > 0 ? 1 : 0) + (selectable ? 1 : 0) + (rowReorderable ? 1 : 0)} className="p-0 sticky left-0 right-0">
                      <div className="flex items-center gap-2 p-1 cursor-pointer hover:bg-indigo-50 transition-colors w-max" style={{ paddingInlineStart: `${row.depth * 20 + 8}px` }} onClick={() => toggleGroupCollapse(row.groupKey)}>
                        <div className="text-indigo-500">{isRtl ? (isCollapsed ? <ChevronLeft size={12}/> : <ChevronDown size={12}/>) : (isCollapsed ? <ChevronRight size={12}/> : <ChevronDown size={12}/>)}</div>
                        <Layers size={10} className="text-indigo-400" />
                        <span className="text-[10px] font-black text-indigo-900">{row.groupValue}</span>
                        <span className="bg-white text-indigo-600 border border-indigo-200 px-1 py-0.5 rounded text-[8px] font-bold">{row.count} {t('ردیف', 'rows')}</span>
                      </div>
                    </td>
                  </tr>
                );
                ...
                >
                  {rowReorderable && (
                    <td style={{...getStickyStyles('ROW_REORDER_COL', false), backgroundColor: 'inherit'}} className={`p-0 text-center bg-white group-hover:bg-slate-50 ${isSelected ? 'bg-indigo-50' : ''} ${isRtl ? 'border-l border-slate-100' : 'border-r border-slate-100'}`}>
                      <div className="cursor-grab active:cursor-grabbing text-slate-300 hover:text-indigo-500 p-1 w-full flex items-center justify-center">
                        <GripVertical size={12} />
                      </div>
                    </td>
                  )}
                  {selectable && (
                    <td style={{...getStickyStyles('SELECT_COL', false), backgroundColor: 'inherit'}} className={`p-1 text-center bg-white group-hover:bg-slate-50 ${isSelected ? 'bg-indigo-50' : ''} ${isRtl ? 'border-l border-slate-100' : 'border-r border-slate-100'}`}>
                  ...
                  {visibleColumns.map((col) => (
                    <td key={`${row.id || rowIndex}-${col.field}`} style={{...getStickyStyles(col.field), backgroundColor: 'inherit'}} className={`p-1.5 text-[10px] text-slate-700 truncate bg-white group-hover:bg-slate-50 ${isSelected ? 'bg-indigo-50' : ''} ${isRtl ? 'border-l border-slate-100' : 'border-r border-slate-100'}`}>
                    ...
                    <td style={{...getStickyStyles('ACTIONS', true), backgroundColor: 'inherit'}} className={`p-1 text-center shadow-[-4px_0_10px_rgba(0,0,0,0.01)] bg-white group-hover:bg-slate-50 ${isSelected ? 'bg-indigo-50' : ''} border-slate-100`}>
                      <div className="flex items-center justify-center gap-0.5">
                        {actions.map((act, i) => {
                          ...
                          return (
                            <button key={i} onClick={(e) => { e.stopPropagation(); act.onClick(row, rowIndex); }} title={act.tooltip} className={`p-1 rounded text-slate-400 border border-transparent hover:border-slate-200 hover:bg-white hover:shadow-sm transition-all ${actClass}`}>
                              <act.icon size={12} strokeWidth={2} />
                            </button>
                          );
                        })}
                      </div>
              ...
              <tr>
                <td colSpan={visibleColumns.length + (actions.length > 0 ? 1 : 0) + (selectable ? 1 : 0) + (rowReorderable ? 1 : 0)} className="p-8 text-center text-slate-400 text-[11px] font-medium bg-slate-50/50">
                  <div className="flex flex-col items-center justify-center gap-2"><Search size={24} className="text-slate-300" /><span>{t('هیچ داده‌ای برای نمایش یافت نشد.', 'No data found to display.')}</span></div>
                </td>
              </tr>
            )}
          </tbody>
      ...
      <div className="flex flex-wrap items-center justify-between p-1 border-t border-slate-200 bg-slate-50 gap-2 shrink-0 z-50">
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-bold text-slate-500">{t('تعداد:', 'Rows:')}</span>
          <select value={pageSize} onChange={(e) => { setPageSize(Number(e.target.value)); setPage(1); }} className="text-[10px] font-bold bg-white border border-slate-300 rounded px-1 py-0.5 outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500 text-slate-700 cursor-pointer">
            {[10, 20, 50, 100].map(size => <option key={size} value={size}>{size}</option>)}
          </select>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] font-bold text-slate-500 mx-1 hidden sm:block">{t(`${(page - 1) * pageSize + 1} تا ${Math.min(page * pageSize, totalRecords)} از ${totalRecords}`, `${(page - 1) * pageSize + 1} to ${Math.min(page * pageSize, totalRecords)} of ${totalRecords}`)}</span>
          <div className="flex items-center gap-0.5 bg-white border border-slate-200 rounded p-0.5 shadow-sm">
            <button onClick={() => setPage(1)} disabled={page === 1} className="p-0.5 text-slate-500 hover:bg-slate-100 disabled:opacity-30 rounded"><ChevronsRight size={12} className={isRtl ? '' : 'rotate-180'} /></button>
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="p-0.5 text-slate-500 hover:bg-slate-100 disabled:opacity-30 rounded"><ChevronRight size={12} className={isRtl ? '' : 'rotate-180'} /></button>
            <div className="text-[10px] font-black text-indigo-700 px-2 bg-indigo-50 rounded leading-5">{page} <span className="text-indigo-300 mx-0.5">/</span> {totalPages || 1}</div>
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages || totalPages === 0} className="p-0.5 text-slate-500 hover:bg-slate-100 disabled:opacity-30 rounded"><ChevronLeft size={12} className={isRtl ? '' : 'rotate-180'} /></button>
            <button onClick={() => setPage(totalPages)} disabled={page === totalPages || totalPages === 0} className="p-0.5 text-slate-500 hover:bg-slate-100 disabled:opacity-30 rounded"><ChevronsLeft size={12} className={isRtl ? '' : 'rotate-180'} /></button>
          </div>
        </div>
      </div>
    </div>
...
const Tree = ({ data = [], idField = 'id', parentField = 'parentId', displayField = 'title', secondaryField, activeField = 'isActive', selectedId, onSelect, onAddChild, onAddRoot, onDelete, onExport, onImport, onDownloadSample, language = 'fa' }) => {
    ...
    return (
      <div key={node[idField]} className="select-none relative">
        <div 
          ...
          className={`flex items-center gap-2 py-1 px-1 my-0.5 cursor-pointer rounded-lg transition-all border border-transparent group
          ...
          style={{ 
             paddingInlineStart: `${depth * 20 + 4}px`,
             paddingInlineEnd: '4px'
          }}
        >
          {hasChildren ? (
            <div 
              className="w-4 h-4 flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-colors z-10 bg-white rounded border border-slate-200 shadow-sm shrink-0 cursor-pointer"
          ...
          <div className={`${isSelected ? 'text-indigo-600' : 'text-slate-400 group-hover:text-indigo-400 transition-colors'} shrink-0 ${!isNodeActive ? 'opacity-60' : ''}`}>
            {hasChildren ? (isExpanded ? <FolderOpen size={12} /> : <Folder size={12} />) : <FileText size={12} />}
          </div>

          {/* Node Content */}
          <div className={`flex items-center gap-1.5 truncate flex-1 min-w-0 ${!isNodeActive ? 'opacity-60' : ''}`}>
             {secondaryField && node[secondaryField] && (
                <span className="font-mono text-[10px] font-bold bg-white/60 border border-slate-200/50 px-1 rounded shrink-0">
                  <HighlightText text={node[secondaryField]} term={searchTerm} />
                </span>
             )}
             <span className={`text-[11px] truncate ${isSelected ? 'font-bold' : ''}`}>
                <HighlightText text={node[displayField]} term={searchTerm} />
             </span>
             {!isNodeActive && (
               <span className="text-[8px] px-1 py-0.5 rounded bg-slate-200/50 text-slate-500 border border-slate-300 shrink-0 font-bold mx-1">
                 {t('غیرفعال', 'Inactive')}
               </span>
             )}
          </div>
          ...
          <div className={`flex items-center gap-0.5 shrink-0 transition-opacity ${isSelected ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
            {onAddChild && <button onClick={(e) => { e.stopPropagation(); onAddChild(node); }} title={t('افزودن زیرمجموعه', 'Add Child')} className="p-1 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded"><Plus size={12}/></button>}
            {onDelete && <button onClick={(e) => { e.stopPropagation(); onDelete(node); }} title={t('حذف', 'Delete')} className="p-1 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded"><Trash2 size={12}/></button>}
          </div>
        </div>

        {hasChildren && isExpanded && (
          <div className="overflow-hidden relative">
            <div className={`absolute top-0 bottom-2 w-px bg-slate-200`} style={{ [isRtl ? 'right' : 'left']: `${depth * 20 + 11}px` }}></div>
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-lg shadow-sm flex flex-col font-sans h-full overflow-hidden" dir={isRtl ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between p-1 border-b border-slate-200 bg-slate-50 gap-2 shrink-0 overflow-x-auto custom-scrollbar">
        <div className="flex items-center gap-1 shrink-0">
          {onAddRoot && <Button size="sm" variant="primary" icon={Plus} onClick={onAddRoot} className="h-7 px-2 text-[10px] shadow-sm">{t('افزودن ریشه', 'Add Root')}</Button>}
          <div className="w-px h-4 bg-slate-200 mx-1"></div>
          <button onClick={expandAll} title={t('باز کردن همه', 'Expand All')} className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-white border border-transparent hover:border-slate-200 rounded transition-all"><Maximize2 size={12}/></button>
          <button onClick={collapseAll} title={t('بستن همه', 'Collapse All')} className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-white border border-transparent hover:border-slate-200 rounded transition-all"><Minimize2 size={12}/></button>
        </div>
        
        <div className="flex items-center gap-1 shrink-0">
          <div className="relative">
            <Search size={12} className={`absolute top-1/2 -translate-y-1/2 ${isRtl ? 'right-1.5' : 'left-1.5'} text-slate-400`} />
            <input 
              type="text" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
              placeholder={t('جستجو در درخت...', 'Search tree...')}
              className={`w-40 h-7 text-[10px] font-bold bg-white border border-slate-200 rounded outline-none focus:border-indigo-400 focus:ring-1 focus:ring-indigo-400 transition-all ${isRtl ? 'pr-6 pl-2' : 'pl-6 pr-2'}`}
            />
          </div>
          <div className="w-px h-4 bg-slate-200 mx-1 hidden sm:block"></div>
          {onDownloadSample && <button onClick={onDownloadSample} title={t('دانلود نمونه فایل', 'Download Sample')} className="p-1 text-slate-500 hover:text-emerald-600 hover:bg-white border border-transparent hover:border-slate-200 rounded transition-all"><FileDown size={12} /></button>}
          {onImport && (
            <>
              <button onClick={() => document.getElementById('tree-import-input').click()} title={t('ورود از اکسل', 'Import Excel')} className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-white border border-transparent hover:border-slate-200 rounded transition-all"><Upload size={12} /></button>
          ...
          {onExport && <button onClick={onExport} title={t('خروجی اکسل', 'Export Excel')} className="p-1 text-slate-500 hover:text-emerald-600 hover:bg-white border border-transparent hover:border-slate-200 rounded transition-all"><FileSpreadsheet size={12} /></button>}
        </div>
      </div>

      <div className="flex-1 overflow-auto custom-scrollbar p-1.5 bg-white">
        {treeData.length > 0 ? treeData.map(node => renderNode(node, 0)) : (
          <div className="flex flex-col items-center justify-center h-full gap-2 text-slate-400 text-[11px] font-medium p-6">
    ...
    <div className="bg-white border border-slate-200 rounded-lg shadow-sm flex flex-col font-sans h-full overflow-hidden" dir={isRtl ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between p-1 border-b border-slate-200 bg-slate-50 gap-2 shrink-0 overflow-x-auto custom-scrollbar">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <div className="relative flex items-center h-full" ref={colMenuRef}>
            <button onClick={() => setShowColMenu(!showColMenu)} title={t('نمایش/مخفی‌سازی ستون‌ها', 'Columns')} className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-white border border-transparent hover:border-slate-200 rounded transition-all h-full flex items-center justify-center"><Settings size={12} /></button>
            {showColMenu && (
              <div className="absolute top-full mt-1 bg-white border border-slate-200 shadow-xl rounded-lg p-2 z-50 min-w-[200px] right-0 animate-in zoom-in-95 duration-100">
                <div className="text-[11px] font-black text-slate-800 mb-2 pb-2 border-b border-slate-100 px-1">{t('نمایش / مخفی‌سازی', 'Show / Hide')}</div>
                <div className="max-h-[250px] overflow-y-auto custom-scrollbar space-y-0.5">
          ...
          {onImport && (
            <>
              <button onClick={() => document.getElementById('treegrid-import-input').click()} title={t('ورود از اکسل', 'Import Excel')} className="p-1 text-slate-500 hover:text-indigo-600 hover:bg-white border border-transparent hover:border-slate-200 rounded transition-all"><Upload size={12} /></button>
      ...
      <div className="overflow-auto custom-scrollbar flex-1 relative bg-white">
        <table className="w-full text-start border-separate border-spacing-0 min-w-max" dir={isRtl ? 'rtl' : 'ltr'}>
          <thead className="sticky top-0 z-40 bg-slate-100 shadow-sm">
            <tr>
              {selectable && (
                <th className={`p-1 border-b border-slate-200 text-center bg-slate-100 w-8 sticky ${isRtl ? 'right-0' : 'left-0'} z-50 ${isRtl ? 'border-l' : 'border-r'}`}>
              ...
              {visibleColumns.map((col, index) => (
                <th key={col.field} style={{ width: col.width || '150px' }} className={`p-1.5 border-b border-slate-200 text-[10px] font-black text-slate-700 bg-slate-100 ${isRtl ? 'border-l' : 'border-r'}`}>
                  {t(col.header_fa, col.header_en)}
                </th>
              ))}
              {actions.length > 0 && (
                <th className={`p-1.5 border-b border-slate-200 text-[10px] font-black text-slate-700 w-[100px] bg-slate-100 text-center sticky ${isRtl ? 'left-0' : 'right-0'} z-50 shadow-[-4px_0_10px_rgba(0,0,0,0.03)]`}>
                  {t('عملیات', 'Actions')}
                </th>
              )}
            </tr>
          ...
          <tbody className="z-10 relative">
            {flatData.length > 0 ? flatData.map((row, rowIndex) => {
              ...
              return (
                ...
                >
                  {selectable && (
                    <td className={`p-1 text-center bg-inherit sticky ${isRtl ? 'right-0' : 'left-0'} z-20 ${isRtl ? 'border-l border-slate-100' : 'border-r border-slate-100'}`}>
                  ...
                  {visibleColumns.map((col, colIndex) => (
                    <td key={col.field} className={`py-1 px-1.5 text-[10px] text-slate-700 truncate bg-inherit ${isRtl ? 'border-l border-slate-100' : 'border-r border-slate-100'}`}>
                      
                      {colIndex === 0 ? (
                        <div className="flex items-center gap-1.5 relative" style={{ paddingInlineStart: `${row._depth * 16}px` }}>
                          {hasChildren ? (
                            <div 
                              className="w-4 h-4 flex items-center justify-center text-slate-400 hover:text-indigo-600 transition-colors z-10 bg-white rounded border border-slate-200 shadow-sm shrink-0 cursor-pointer"
                          ...
                          {isEditing && col.editable ? (
                            <input 
                              ...
                              className="h-6 text-[10px] border border-indigo-300 focus:ring-1 focus:ring-indigo-300 rounded px-1 w-full"
                            />
                          ) : (
                            <span className={`text-[11px] truncate ${isSelectedRow ? 'font-bold text-indigo-900' : 'text-slate-800'}`}>
                        ...
                        isEditing && col.editable ? (
                          col.type === 'select' ? (
                            <select 
                              value={editData[col.field] || ''} 
                              onChange={(e) => onEditFieldChange(col.field, e.target.value)} 
                              className="h-6 text-[10px] border border-indigo-300 focus:ring-1 focus:ring-indigo-300 rounded px-1 w-full outline-none"
                            >
                              <option value="">...</option>
                    ...
                    <td className={`p-1 text-center shadow-[-4px_0_10px_rgba(0,0,0,0.01)] bg-inherit sticky ${isRtl ? 'left-0' : 'right-0'} z-20 border-slate-100`}>
                      {isEditing ? (
                        <div className="flex items-center justify-center gap-1 opacity-100">
                          <button onClick={(e) => { e.stopPropagation(); onSaveEdit(row); }} title={t('ذخیره', 'Save')} className="p-1 rounded text-emerald-600 bg-emerald-50 hover:bg-emerald-100 transition-all">
                            <Check size={12} strokeWidth={2} />
                          </button>
                          <button onClick={(e) => { e.stopPropagation(); onCancelEdit(); }} title={t('انصراف', 'Cancel')} className="p-1 rounded text-red-600 bg-red-50 hover:bg-red-100 transition-all">
                        ...
                        <div className="flex items-center justify-center gap-0.5 opacity-100">
                          {onAddChild && (
                            <button onClick={(e) => { e.stopPropagation(); onAddChild(row); }} title={t('افزودن زیرمجموعه', 'Add Child')} className="p-1 rounded text-slate-400 hover:border-slate-200 hover:bg-white hover:text-emerald-600 hover:shadow-sm transition-all">
                              <Plus size={12} strokeWidth={2} />
                            </button>
                          )}
                          {actions.map((act, i) => (
                            <button key={i} onClick={(e) => { e.stopPropagation(); act.onClick(row, rowIndex); }} title={act.tooltip} className={`p-1 rounded text-slate-400 border border-transparent hover:border-slate-200 hover:bg-white hover:shadow-sm transition-all ${act.className || 'hover:text-indigo-600'}`}>
                              <act.icon size={12} strokeWidth={2} />
                            </button>
                          ))}
                          {onDelete && (
                            <button onClick={(e) => { e.stopPropagation(); onDelete(row); }} title={t('حذف', 'Delete')} className="p-1 rounded text-slate-400 hover:border-slate-200 hover:bg-white hover:text-red-600 hover:shadow-sm transition-all">
              ...
              <tr>
                <td colSpan={columns.length + (actions.length > 0 ? 1 : 0) + (selectable ? 1 : 0)} className="p-8 text-center text-slate-400 text-[11px] font-medium bg-slate-50/50">
                  <div className="flex flex-col items-center justify-center gap-2"><Layers size={24} className="text-slate-300" /><span>{t('هیچ داده‌ای برای نمایش یافت نشد.', 'No data found to display.')}</span></div>
                </td>
              </tr>
            )}
          </tbody>
  ...
  return (
    <div className={`flex flex-col gap-1.5 w-full`}>
      ...
      <textarea
        ...
        className={`w-full bg-white border rounded-lg text-slate-800 transition-all outline-none p-2 text-[11px] placeholder:text-slate-400 focus:bg-white focus:ring-2 ${disabled ? 'bg-slate-100/50 text-slate-500 border-slate-200' : 'border-slate-300 focus:border-indigo-400 focus:ring-indigo-100 hover:border-slate-400'} ${className}`}
        dir={isRtl ? 'rtl' : 'ltr'} {...props}
      />
      {error && <div className="flex items-center gap-1 text-red-500 text-[10px] font-bold mt-0.5"><AlertCircle size={10} /><span>{error}</span></div>}
    </div>
        ...
        {options.map((opt) => (
          <label key={opt.value} className="flex items-center gap-2 cursor-pointer group">
            <div className="relative flex items-center justify-center">
              ...
              />
              <div className={`w-3.5 h-3.5 rounded-full border transition-all ${value === opt.value ? 'border-indigo-600 bg-white' : 'border-slate-300 bg-white group-hover:border-slate-400'}`}></div>
              {value === opt.value && <div className="absolute w-1.5 h-1.5 rounded-full bg-indigo-600 animate-in zoom-in-50 duration-200"></div>}
            </div>
...
const EmptyState = ({ title, description, icon: Icon = Search, action }) => {
  return (
    <div className="flex flex-col items-center justify-center p-6 text-center animate-in fade-in duration-500">
      <div className="w-14 h-14 bg-slate-100 rounded-full flex items-center justify-center text-slate-300 mb-4">
...
const StatCard = ({ label, value, icon: Icon, trend, trendValue, color = 'indigo', language = 'fa' }) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-xl p-3 shadow-sm group hover:border-indigo-200 transition-all">
      <div className="flex items-start justify-between mb-2">
        <div className={`p-1.5 rounded-lg border ${colors[color]} group-hover:scale-110 transition-transform`}>
          {Icon && <Icon size={16} />}
        </div>
        {trend && (
          <div className={`flex items-center gap-0.5 text-[9px] font-black px-1.5 py-0.5 rounded-full ${trend === 'up' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'}`}>
      ...
      <div className="flex flex-col">
        ...
        <span className="text-[16px] font-black text-slate-800 tracking-tight">{value}</span>
      </div>
    </div>
  );
};
...
const Timeline = ({ items = [], language = 'fa' }) => {
  const isRtl = language === 'fa';
  return (
    <div className="flex flex-col gap-3 relative py-2" dir={isRtl ? 'rtl' : 'ltr'}>
      <div className={`absolute top-0 bottom-0 w-0.5 bg-slate-100 ${isRtl ? 'right-1.5' : 'left-1.5'}`}></div>
      {items.map((item, idx) => (
        <div key={idx} className="flex gap-3 relative z-10">
          <div className={`w-3.5 h-3.5 rounded-full border-2 border-white shadow-sm shrink-0 mt-1 ${item.variant === 'success' ? 'bg-emerald-500' : item.variant === 'danger' ? 'bg-rose-500' : 'bg-indigo-500'}`}></div>
          <div className="flex flex-col gap-0.5 min-w-0 flex-1">
            ...
            {item.description && <p className="text-[10px] text-slate-500 leading-relaxed">{item.description}</p>}
          </div>
        </div>
      ))}
    </div>
...
const Avatar = ({ src, name, size = 'md', className = '' }) => {
  const sizes = { sm: 'w-6 h-6 text-[9px]', md: 'w-8 h-8 text-[11px]', lg: 'w-12 h-12 text-[13px]' };
...
const DropdownMenu = ({ trigger, items = [], language = 'fa' }) => {
  ...
  return (
    <div className="relative inline-block text-start" ref={ref} dir={isRtl ? 'rtl' : 'ltr'}>
      ...
      {open && (
        <div className={`absolute z-[150] mt-1 w-44 bg-white border border-slate-200 shadow-xl rounded-xl p-1 animate-in zoom-in-95 duration-150 ${isRtl ? 'right-0' : 'left-0'}`}>
          {items.map((item, i) => (
            item.divider ? <div key={i} className="h-px bg-slate-100 my-1 mx-1"></div> :
            <button 
              key={i} 
              onClick={() => { item.onClick?.(); setOpen(false); }}
              className={`flex items-center gap-2 w-full px-2.5 py-1.5 text-[10px] font-bold rounded-lg transition-colors ${item.variant === 'danger' ? 'text-rose-500 hover:bg-rose-50' : 'text-slate-700 hover:bg-slate-50'}`}
            >
...
const ProgressBar = ({ value = 0, max = 100, color = 'indigo', size = 'md', label, showValue = true }) => {
  ...
  const heights = { sm: 'h-1', md: 'h-1.5', lg: 'h-3' };
...
const DatePicker = ({ label, value, onChange, isRtl = true, language = 'fa', required = false, size = 'md', disabled = false, id, wrapperClassName = '' }) => {
  ...
  const heights = { sm: 'h-7 text-[11px]', md: 'h-8 text-[11px]', lg: 'h-10 text-[12px]' };
  ...
  return (
    <div className={`flex flex-col ${size === 'sm' ? 'gap-1' : 'gap-1.5'} w-full ${wrapperClassName}`}>
      ...
      <div className="relative flex items-center group">
        <div className={`absolute ${isRtl ? 'right-2' : 'left-2'} text-slate-400 group-hover:text-indigo-500 transition-colors pointer-events-none`}>
          <Calendar size={14} />
        </div>
        <input 
...
const Stepper = ({ steps = [], currentStep = 0, language = 'fa' }) => {
  const isRtl = language === 'fa';
  return (
    <div className="w-full py-2 px-2" dir={isRtl ? 'rtl' : 'ltr'}>
      <div className="flex items-center justify-between relative">
        <div className="absolute top-3 left-0 right-0 h-0.5 bg-slate-100 z-0"></div>
        {steps.map((step, idx) => {
          const isActive = idx === currentStep;
          const isCompleted = idx < currentStep;
          return (
            <div key={idx} className="flex flex-col items-center gap-1.5 relative z-10 flex-1">
              <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all duration-300 ${isCompleted ? 'bg-emerald-500 border-emerald-500 text-white' : isActive ? 'bg-white border-indigo-600 text-indigo-600 scale-110 shadow-md' : 'bg-white border-slate-200 text-slate-300'}`}>
                {isCompleted ? <Check size={12} strokeWidth={3} /> : <span className="text-[10px] font-black">{idx + 1}</span>}
              </div>
              <span className={`text-[9px] font-black transition-colors ${isActive ? 'text-indigo-600' : isCompleted ? 'text-emerald-600' : 'text-slate-400'}`}>{step.label}</span>
            </div>
          );
        })}
      </div>
...
const TagInput = ({ tags = [], onAdd, onDelete, placeholder, label, isRtl = true, size = 'md', wrapperClassName = '' }) => {
  ...
  const minHeights = { sm: 'min-h-[28px]', md: 'min-h-[32px]', lg: 'min-h-[40px]' };
  
  return (
    <div className={`flex flex-col ${size === 'sm' ? 'gap-1' : 'gap-1.5'} w-full ${wrapperClassName}`}>
      {label && <label className="text-[11px] font-bold text-slate-700">{label}</label>}
      <div className={`flex flex-wrap gap-1 p-1 bg-white border border-slate-300 rounded-lg focus-within:border-indigo-400 focus-within:ring-2 focus-within:ring-indigo-100 transition-all items-center ${minHeights[size]}`} dir={isRtl ? 'rtl' : 'ltr'}>
        {tags.map((tag, idx) => (
          <div key={idx} className="flex items-center gap-1 bg-indigo-50 text-indigo-700 px-1.5 py-0.5 rounded border border-indigo-100 animate-in zoom-in-90 duration-150">
        ...
        <input 
          ...
          className="flex-1 min-w-[60px] h-6 bg-transparent border-none outline-none text-[11px] placeholder:text-slate-400 px-1"
        />
      </div>
    </div>
  );
