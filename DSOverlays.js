...
(() => {
  ...
  const { X, ChevronRight, ChevronLeft } = window.LucideIcons || {};
  ...
  const Drawer = ({ isOpen, onClose, title, children, position = 'right', width = 'max-w-sm', language = 'fa' }) => {
    ...
    return (
      <div className="fixed inset-0 z-[150] font-sans" dir={isRtl ? 'rtl' : 'ltr'}>
        <div className="absolute inset-0 bg-slate-900/20 backdrop-blur-[2px] animate-in fade-in duration-300" onClick={onClose} />
        <aside 
          className={`absolute top-0 bottom-0 ${width} w-full bg-white/95 backdrop-blur-2xl shadow-[0_0_60px_rgba(0,0,0,0.1)] border-slate-200/60 flex flex-col transition-all duration-300 ${placementClasses} ${slideAnim}`}
        >
          <div className="h-12 border-b border-slate-100 flex items-center justify-between px-4 bg-slate-50/50 shrink-0">
            <h3 className="font-black text-slate-700 text-[13px] tracking-tight truncate">{title}</h3>
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar p-4 bg-white flex flex-col min-h-0">
            {children}
          </div>
        </aside>
      </div>
      ...
      <div onContextMenu={handleContextMenu} className="relative w-full h-full">
        ...
        {isVisible && (
          <div 
            ...
            className="fixed z-[9999] w-48 bg-white/95 backdrop-blur-xl border border-slate-200/80 shadow-[0_10px_40px_rgba(0,0,0,0.1)] rounded-xl py-1.5 animate-in fade-in zoom-in-95 duration-150 font-sans"
            dir={isRtl ? 'rtl' : 'ltr'}
          >
            {items.map((item, idx) => {
              if (item.divider) return <div key={idx} className="h-px bg-slate-100 my-1 mx-2" />;
              return (
                <button
                  key={idx}
    ...
    return (
      <div className="relative inline-flex font-sans" dir={isRtl ? 'rtl' : 'ltr'}>
        ...
        {isVisible && (
          <div 
            ref={popoverRef} 
            className={`absolute z-[100] ${posClasses[position]} bg-white/95 backdrop-blur-2xl border border-slate-200/80 shadow-[0_20px_60px_rgba(0,0,0,0.12)] rounded-2xl p-3 min-w-[200px] animate-in fade-in zoom-in-95 duration-200`}
          >
            {children}
          </div>
        )}
