...
(() => {
  ...
  const ChartTooltip = ({ visible, x, y, content }) => {
    ...
    return (
      <div 
        className="fixed z-[9999] pointer-events-none bg-slate-800 text-white text-[11px] font-bold px-3 py-1.5 rounded-lg shadow-xl transform -translate-x-1/2 -translate-y-full mt-[-10px] animate-in fade-in duration-150" 
        style={{ left: x, top: y }}
      >
        {content}
            ...
            {data.map((d, i) => {
              ...
              return (
                <g 
                  ...
                  onMouseMove={(e) => setTooltip({ visible: true, x: e.clientX, y: e.clientY, content: `${d.label}: ${d.value.toLocaleString()}` })}
                  onMouseLeave={() => setTooltip({ visible: false, x: 0, y: 0, content: '' })}
                  onClick={() => onClick && onClick(d)}
                >
                  <circle cx={x} cy={y} r="4" fill={theme.main} stroke="white" strokeWidth="1.5" className="transition-all duration-200 group-hover:r-5" />
      ...
      <div className="w-full flex items-center justify-center font-sans gap-6 relative" style={{ height }} dir={isRtl ? 'rtl' : 'ltr'}>
        <div className="relative h-full aspect-square max-h-[160px]">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
             <span className="text-[10px] text-slate-400 font-bold">{hoveredSeg ? hoveredSeg.label : t('مجموع', 'Total')}</span>
